...
penguins %>%
  ...
  pivot_wider(names_from = island,
              values_from = n,
              values_fill = 0) # inserting a 0 for the NA values in our table
...
# to get the *column* proportions we will group by island
penguins %>%
  count(species, island) %>% 
  group_by(island) %>% 
  mutate(prop = n / sum(n))
...
ggplot(penguins, aes(species, fill = island)) + # fill colors by the proportions
  geom_bar()
...
A histogram represents continuous data. Where the data is binned into bars (bar width represents these bins). Bar plots are used when our independent variables are categorical data.
...
```{r}
bears <- read_csv(here::here("grizzlyBear_behaviorDiet_northSlopeAlaska_2023.csv")) %>%
  janitor::clean_names() %>%
  filter(bear == c('198', '200', '202')) %>%
  filter(behavior == c('eating', 'walking', 'laying', 'digging'))

```

```{r}

table(bears$bear, bears$behavior)
...
bears_table <- table(bears$bear, bears$behavior)
addmargins(bears_table)

```

```{r}
# Row proportions
prop.table(bears_table, margin = 1)
# Column proportions
prop.table(bears_table, margin = 2)
```
...
ggplot(bears, aes(as.factor(bear), fill = behavior)) + # fill colors by the proportions
  geom_bar() + 
  scale_fill_manual(values = c("dodgerblue4", "darkolivegreen4", "darkorchid3", "goldenrod1"),
                    labels = c("Digging", "Eating", "Laying", "Walking")) +
  labs(title = "Observed bear behaviors on the North Slope of Alaska, USA, 2023",
       x = "Bear ID",
